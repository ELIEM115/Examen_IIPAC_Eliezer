...
namespace Datos
{
    ...
    {
        string cadena = "server=localhost; user=root; database=ticket; password=1105;";
                    ...
                    try
                    {
                        ...
                        foreach (DetalleTicket detalle in detalles)
                        {
                            using (MySqlCommand cmd2 = new MySqlCommand(sqlDetalle.ToString(), con, transaction))
                            {
                                ...
                                cmd2.Parameters.Add("@TipoSoporte", MySqlDbType.VarChar, 25).Value = detalle.TipoSoporte;
                                cmd2.Parameters.Add("@DescripcionSolicitud", MySqlDbType.VarChar, 45).Value = detalle.DescripcionSolicitud;
                                cmd2.Parameters.Add("@DescripcionRespuesta", MySqlDbType.VarChar, 45).Value = detalle.DescripcionRespuesta;
